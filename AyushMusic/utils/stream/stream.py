...
import config
from AyushMusic import Carbon, YouTube, app
from AyushMusic.core.call import Aru
from AyushMusic.misc import db
from AyushMusic.utils.database import add_active_video_chat, is_active_chat
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await Aru.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Aru.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Aru.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
