...
async def fork_repo_callback(client, query):
    await query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("🚀 Fᴏʀᴋ Rᴇᴘᴏ", url="https://github.com/NoxxOP/AyushMusic/fork"),
                    InlineKeyboardButton("⚡ Hᴇʀᴏᴋᴜ Dᴇᴘʟᴏʏ", url="https://dashboard.heroku.com/new?template=https://github.com/NoxxOP/AyushMusic")
                ],
                [
...
from AyushMusic.utils.inline.start import about_panel
from strings import get_string
...
from pyrogram import Client, filters
from pyrogram.types import InlineKeyboardMarkup
from AyushMusic.utils.inline.start import owner_panel  # Import owner_panel function
from strings import get_string
from config import BANNED_USERS
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Aru.pause_stream(chat_id)
        ...
        await Aru.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Aru.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Aru.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                await Aru.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await Aru.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            ...
            try:
                await Aru.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
