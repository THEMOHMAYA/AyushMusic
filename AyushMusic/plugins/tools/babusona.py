...
@app.on_message(
    filters.command(["gali", ".gali"], prefixes=["/", "."]) & filters.private
)
async def gali_private(client: Client, message: Message):
    await message.reply_text(
        text=random.choice(GALI),
    )

@app.on_message(filters.command(["gali", ".gali"]) & filters.group)
async def gali_group(client: Client, message: Message):
    ...
    if member.status in ["administrator", "creator"]:
        if message.reply_to_message:
            reply_msg = await message.reply(f"{message.reply_to_message.from_user.mention} {random.choice(GALI)}")
        else:
            reply_msg = await message.reply(random.choice(GALI))
...
This command provides random abusive language (gali) when used in private chats. In groups, only admins can use it.

Features:
- Works in bot's DM for any user
- Only group admins/owners can use in groups
- Auto-deletes after 1 minute in groups
- Supports both /gali and .gali commands
- Use Own own risk owner is not responsible for this 
